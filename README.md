...
**エディオンピースウィング広島**（2024年2月開業のサッカー専用スタジアム）は、**PLATEAU の建築物モデルに含まれていません**。該当メッシュの CityGML (`51324376_bldg_6697_op.gml`) を確認したところ、この位置の半径 250m 以内で最大の建物は 54m × 64m で、スタジアム（約 200m × 160m）は存在しませんでした。FY2024 のデータですが、航空測量の時点が開業前だったとみられます。そのため原爆ドームと同じく独自にモデリングしています（角の丸い長方形のボウル、木調ルーバーの外周、屋根、客席、ピッチ、照明塔）。寸法は概略値です。
