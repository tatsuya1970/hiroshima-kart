...
タイトル画面で「対戦PLAY」を押すと公開ロビーに入り、**30 秒のカウントダウンで自動的に発走**します。集まった人どうしで最大 8 人、空いた枠は AI が走ります。誰も来なくても待たされません。
...
**トップ画面に「対戦待ち」の状況を出します。** 対戦PLAY を押す前から、レースの部屋とは別の常設の部屋（`hk-presence`）に全員が入り、「トップ画面にいる / 対戦待ち / レース中」を伝え合います（`src/net.ts` の `Presence`）。誰かが待っていれば「いま 1 人が対戦待ち（たろう）発走まで 18 秒」と緑で光り、ロビーで待っている側にも「トップ画面に 1 人います」と出ます。

trystero 0.25 は同じ appId なら部屋をまたいで WebRTC 接続を共有します（`@trystero-p2p/core` の SharedPeerManager）。そのため、トップ画面でつながった相手とは、対戦PLAY を押した瞬間にリレーの往復なしで同じ部屋に入れます。相手とつながるまでの 8〜19 秒はページの読み込み中に済み、対戦待ちの人が見えていれば締切の 3 秒前（`JOIN_MIN_WAIT`）までその部屋に入れます。「12 秒前を過ぎたら次の区間」に回すのは、対戦待ちの人が見えていないときだけです。
...
`PORT=5181 node tools/nettest.mjs` でブラウザを 2 つ立ち上げ、部屋作成・参加・開始・位置の一致まで通しで確認できます。`PORT=5181 node tools/presencetest.mjs` は、トップ画面に対戦待ちが出るか、その状態で押すと締切 8 秒前でも同じ部屋に即座に席が決まるかを確認します。
