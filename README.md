...
**エディオンピースウィング広島**（2024年2月開業のサッカー専用スタジアム）は、**PLATEAU の建築物モデルに含まれていません**。該当メッシュの CityGML (`51324376_bldg_6697_op.gml`) を確認したところ、この位置の半径 250m 以内で最大の建物は 54m × 64m で、スタジアム（約 200m × 160m）は存在しませんでした。FY2024 のデータですが、航空測量の時点が開業前だったとみられます。そのため原爆ドームと同じく独自にモデリングしています（角の丸い長方形のボウル、木調ルーバーの外周、屋根、客席、ピッチ、照明塔）。位置・向き・寸法は概略値です。

なお中央公園のこの一帯は平坦で標高が低く、地形生成が河川と誤判定します（DEM の低標高部を水面として扱っているため。スタジアム中心から半径 100m の外周 36 点中 19 点が水判定）。そのままだと足元が水面になるので、敷地の高さは周囲の陸地の中央値から取り、下に敷地の平場を敷いています。一帯の水判定そのものを直すには地形の再生成が必要です。
