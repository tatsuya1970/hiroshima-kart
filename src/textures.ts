...
export function makeBannerTexture(): THREE.Texture {
  ...
  ctx.fillStyle = '#c1121f'; ctx.fillRect(0, 0, 1024, 256);
  ...
  ctx.fillText('HIROSHIMA KART', 512, 118);
  ctx.font = 'bold 44px sans-serif'; ctx.fillStyle = '#ffd83d'; ctx.fillText('広島グランプリ  START / FINISH', 512, 196);
  return toTexture(c, false);
}

/** 車両側面 (1 タイル = 車体 1 両分の側面) */
export function makeTrainSideTexture(kind: 'tram' | 'jr' | 'shinkansen' | 'astram'): THREE.Texture {
  const [c, ctx] = canvas(1024, 256);
  const body = kind === 'shinkansen' ? '#f4f6f8' : '#f2f5f7';
  ctx.fillStyle = body; ctx.fillRect(0, 0, 1024, 256);
  const glass = '#1d2b38';
  const bandColor = kind === 'tram' ? '#2f8f52' : kind === 'jr' ? '#c8102e' : kind === 'astram' ? '#1a7fc1' : '#1b4f9c';
  // 帯
  if (kind === 'shinkansen') {
    ctx.fillStyle = bandColor; ctx.fillRect(0, 168, 1024, 26);
    ctx.fillStyle = '#8ec0e8'; ctx.fillRect(0, 194, 1024, 8);
  } else if (kind === 'jr') {
    ctx.fillStyle = bandColor; ctx.fillRect(0, 60, 1024, 18);
    ctx.fillRect(0, 176, 1024, 22);
  } else if (kind === 'astram') {
    ctx.fillStyle = bandColor; ctx.fillRect(0, 172, 1024, 20);
    ctx.fillStyle = '#5cbf6a'; ctx.fillRect(0, 192, 1024, 10);
  } else {
    ctx.fillStyle = bandColor; ctx.fillRect(0, 178, 1024, 34);
    ctx.fillStyle = '#d8ecdd'; ctx.fillRect(0, 54, 1024, 10);
  }
  // 窓とドア
  const winTop = 78, winH = 88;
  const doorW = 84, winW = 108;
  let x = 26;
  const doorMat = kind === 'shinkansen' ? body : '#dfe4e8';
  while (x < 1024 - 26) {
    // ドア
...
/** 原爆ドームの煉瓦 (被爆で焼けた色調) */
export function makeDomeBrickTexture(): THREE.Texture {
  const [c, ctx] = canvas(512, 512);
  ctx.fillStyle = '#9d8b76'; ctx.fillRect(0, 0, 512, 512);
  for (let y = 0; y < 512; y += 16) {
    for (let x = (y / 16 % 2) * 18 - 18; x < 512; x += 36) {
      const r = 150 + Math.random() * 30 | 0, g = 128 + Math.random() * 26 | 0, b = 108 + Math.random() * 24 | 0;
      ctx.fillStyle = `rgb(${r},${g},${b})`;
      ctx.fillRect(x + 1, y + 1, 34, 14);
    }
  }
  // 汚れ・焼け跡
  for (let i = 0; i < 70; i++) {
    ctx.fillStyle = `rgba(60,48,38,${0.05 + Math.random() * 0.12})`;
    ctx.beginPath();
    ctx.ellipse(Math.random() * 512, Math.random() * 512, 20 + Math.random() * 60, 12 + Math.random() * 40, Math.random() * 3, 0, Math.PI * 2);
    ctx.fill();
  }
  // 縦の雨だれ
  for (let i = 0; i < 40; i++) {
    const x = Math.random() * 512;
    ctx.fillStyle = 'rgba(70,58,46,0.10)';
    ctx.fillRect(x, Math.random() * 300, 3 + Math.random() * 6, 120 + Math.random() * 180);
  }
  const t = toTexture(c);
  t.repeat.set(1, 1);
  return t;
...
/** 崩落面・基壇の石 */
export function makeRuinStoneTexture(): THREE.Texture {
  const [c, ctx] = canvas(256, 256);
  ctx.fillStyle = '#a79c8c'; ctx.fillRect(0, 0, 256, 256);
  for (let i = 0; i < 1400; i++) {
    const g = 130 + Math.random() * 60 | 0;
    ctx.fillStyle = `rgba(${g},${g - 8},${g - 22},0.5)`;
    ctx.fillRect(Math.random() * 256, Math.random() * 256, 3 + Math.random() * 5, 3 + Math.random() * 5);
  }
  return toTexture(c);
}

/** 広島城の壁面: 上部は白漆喰、下部は黒い下見板張り、格子窓つき */
export function makeCastleWallTexture(): THREE.Texture {
  const [c, ctx] = canvas(512, 256);
  // 白漆喰
  ctx.fillStyle = '#efece4'; ctx.fillRect(0, 0, 512, 256);
  for (let i = 0; i < 900; i++) {
    ctx.fillStyle = `rgba(120,115,105,${Math.random() * 0.07})`;
    ctx.fillRect(Math.random() * 512, Math.random() * 256, 4, 3);
  }
  // 下見板張り (下 4 割)
  const boardTop = 150;
  ctx.fillStyle = '#2e2a26'; ctx.fillRect(0, boardTop, 512, 256 - boardTop);
  for (let y = boardTop; y < 256; y += 11) {
    ctx.fillStyle = 'rgba(255,255,255,0.07)'; ctx.fillRect(0, y, 512, 2);
    ctx.fillStyle = 'rgba(0,0,0,0.35)'; ctx.fillRect(0, y + 9, 512, 2);
  }
  // 白漆喰と板の境の水切り
  ctx.fillStyle = '#d9d4c8'; ctx.fillRect(0, boardTop - 5, 512, 5);
  // 格子窓
  for (let x = 40; x < 512; x += 96) {
    ctx.fillStyle = '#1b1713'; ctx.fillRect(x, 42, 52, 62);
    ctx.strokeStyle = '#efece4'; ctx.lineWidth = 3;
    for (let k = 1; k < 4; k++) { ctx.beginPath(); ctx.moveTo(x + k * 13, 42); ctx.lineTo(x + k * 13, 104); ctx.stroke(); }
    ctx.beginPath(); ctx.moveTo(x, 73); ctx.lineTo(x + 52, 73); ctx.stroke();
    ctx.strokeStyle = '#c9c2b2'; ctx.lineWidth = 4; ctx.strokeRect(x - 3, 39, 58, 68);
  }
  return toTexture(c);
}

/** 本瓦葺きの屋根 */
export function makeCastleRoofTexture(): THREE.Texture {
  const [c, ctx] = canvas(256, 256);
  ctx.fillStyle = '#4a4f55'; ctx.fillRect(0, 0, 256, 256);
  // 平瓦の段
  for (let y = 0; y < 256; y += 16) {
    ctx.fillStyle = 'rgba(0,0,0,0.28)'; ctx.fillRect(0, y, 256, 3);
    ctx.fillStyle = 'rgba(255,255,255,0.10)'; ctx.fillRect(0, y + 3, 256, 2);
  }
  // 丸瓦の筋
  for (let x = 0; x < 256; x += 22) {
    ctx.fillStyle = 'rgba(255,255,255,0.13)'; ctx.fillRect(x, 0, 5, 256);
    ctx.fillStyle = 'rgba(0,0,0,0.22)'; ctx.fillRect(x + 5, 0, 3, 256);
  }
  for (let i = 0; i < 500; i++) {
...
export function makeStoneWallTexture(): THREE.Texture {
  const [c, ctx] = canvas(512, 512);
  ctx.fillStyle = '#6f6a60'; ctx.fillRect(0, 0, 512, 512);
  const rnd = (a: number, b: number) => a + Math.random() * (b - a);
  for (let y = 0; y < 512; y += 44) {
    let x = -rnd(0, 40);
    while (x < 512) {
      const w = rnd(48, 96), h = 42;
      const g = 130 + Math.random() * 45 | 0;
      ctx.fillStyle = `rgb(${g},${g - 6},${g - 16})`;
      ctx.beginPath();
      ctx.moveTo(x + 3, y + 3);
      ctx.lineTo(x + w - rnd(2, 8), y + rnd(2, 6));
      ctx.lineTo(x + w - rnd(2, 6), y + h - rnd(2, 6));
      ctx.lineTo(x + rnd(2, 8), y + h - rnd(2, 5));
      ctx.closePath();
      ctx.fill();
      ctx.strokeStyle = 'rgba(40,36,30,0.55)'; ctx.lineWidth = 3; ctx.stroke();
      x += w;
    }
  }
  for (let i = 0; i < 700; i++) {
