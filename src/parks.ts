// 緑地と水面 — 平和記念公園の芝生・樹木と、広島城の内堀。
//
// PLATEAU にはどちらの地物も無い。建築物 (bldg)・道路 (tran)・地形 (dem) には
// 公園も濠も含まれず、土地利用 (luse) は 250m メッシュを塗っただけなので輪郭に
// 使えない (実測: 平和記念公園の一帯は 6.63ha の正方形 4 枚で覆われるだけ)。
// そのため data/parks.json に実際の輪郭を置き、次の 3 段で再現する。
//
//   1. carve: DEM を掘って濠にし、公園内で誤って水面と判定されている低地は埋める
//   2. draw:  地面テクスチャに芝と水面を描く (道路とコースはこの上に描かれる)
//   3. build: 濠の水面・石垣と、樹木のメッシュを作る
//
// 濠は DEM に部分的にしか入っていない (北と西は 2.0-2.5m の窪みとして残るが、
// 南と東は周囲と同じ 3.5-4.0m で潰れている) ので、全周を掘り直している。
...
export class Parks {
  readonly areas: Area[] = [];
  readonly moatInner: Ring;
  readonly moatOuter: Ring;
  ...
  build(buildings: BuildingsData, track: Track): THREE.Group {
    ...
    g.add(this.buildMoat());
    g.add(this.buildTrees(buildings, track));
    return g;
  }

  /** 濠: 水面の帯 + 内外の石垣 */
  private buildMoat(): THREE.Group {
    const g = new THREE.Group();
    const n = this.moatInner.length / 2;
    // ---- 水面 (inner と outer の間を帯で張る) ----
    const pos: number[] = [], idx: number[] = [];
    for (let i = 0; i <= n; i++) {
      const k = (i % n) * 2;
      pos.push(this.moatInner[k], this.waterLevel, this.moatInner[k + 1]);
      pos.push(this.moatOuter[k], this.waterLevel, this.moatOuter[k + 1]);
    ...
    for (const ring of [this.moatInner, this.moatOuter]) {
      const p: number[] = [], ix: number[] = [];
      for (let i = 0; i <= n; i++) {
        const k = (i % n) * 2;
        const x = ring[k], z = ring[k + 1];
        // 岸の高さ: 帯の中は掘ってあるので、帯の外側 (中心と反対) 8m の地形を見る
        const cx = (this.moatInner[k] + this.moatOuter[k]) / 2, cz = (this.moatInner[k + 1] + this.moatOuter[k + 1]) / 2;
        let dx = x - cx, dz = z - cz;
