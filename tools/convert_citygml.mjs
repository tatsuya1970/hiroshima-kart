// PLATEAU CityGML (広島市 2024) → ゲーム用データ変換
...
import { readFileSync, writeFileSync, createReadStream, readdirSync } from 'node:fs';
...
// 出力範囲 (コース + 余白)。北端はアストラムライン地上部・牛田まで含める。
const BB = { latMin: 34.386, latMax: 34.4150, lonMin: 132.443, lonMax: 132.491 };
...
const buildings = [];
for (const f of files.filter(f => f.endsWith('_bldg.gml'))) {
  const t = readFileSync(path.join(DIR, f), 'utf8');
  const b = parseBuildings(t);
  buildings.push(...b);
  console.log('bldg ' + f + ': ' + b.length);
}
writeFileSync('public/data/buildings.json', JSON.stringify({ origin: course.origin, count: buildings.length, items: buildings }));
console.log('buildings total', buildings.length, `(ランドマーク位置で除外 ${domeSkipped} 棟)`);

// ---------- 道路 ----------
const roads = [];
for (const f of files.filter(f => f.endsWith('_tran.gml'))) {
  const t = readFileSync(path.join(DIR, f), 'utf8');
  let idx = 0, c = 0;
  while (true) {
...
writeFileSync('public/data/roads.json', JSON.stringify({ items: roads }));
...
const demFile = path.join(DIR, '513243_dem_55.gml');
let rest = '';
let tri = 0;
...
// 水面判定: DEM 欠損 or 標高 0.4m 未満を「確実な水面」とし、
//   標高 1.0m 未満のセルは確実な水面に連結していれば水面 (干潟状の猿猴川などを拾う)
//   標高 1.0-1.6m のセルは水面から 8 セル (40m) 以内に限り水面 (河床の縁)
const raw = new Float32Array(W * H);
const isWater = new Uint8Array(W * H); // 1 = 確定, 2 = 候補(低), 3 = 候補(縁)
for (let k = 0; k < W * H; k++) {
  if (hcnt[k] === 0) { isWater[k] = 1; raw[k] = 0; continue; }
  const h = hsum[k] / hcnt[k];
  raw[k] = h;
  if (h < 0.4) isWater[k] = 1; else if (h < 1.0) isWater[k] = 2; else if (h < 1.6) isWater[k] = 3;
}
const budget = new Uint8Array(W * H);
const stack = [];
for (let k = 0; k < W * H; k++) if (isWater[k] === 1) { budget[k] = 8; stack.push(k); }
while (stack.length) {
  const k = stack.pop();
  const i = k % W, j = (k - i) / W;
  for (let dj = -1; dj <= 1; dj++) for (let di = -1; di <= 1; di++) {
    ...
    const kk = jj * W + ii;
    if (isWater[kk] === 2) { isWater[kk] = 1; budget[kk] = 8; stack.push(kk); }
    else if (isWater[kk] === 3 && budget[k] > 1) { isWater[kk] = 1; budget[kk] = budget[k] - 1; stack.push(kk); }
  }
}
