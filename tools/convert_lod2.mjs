...
const CORRIDOR = Number(process.env.LOD2_CORRIDOR ?? 200);
// アトラス設定
const ATLAS = 4096, CELL = 256, PAD = 16, IMG = CELL - PAD * 2; // 224px
const PER_ROW = Math.floor(ATLAS / CELL), PER_ATLAS = PER_ROW * PER_ROW;

const DIR = 'data/citygml';
const MESHES = ['51324366', '51324367', '51324368', '51324376', '51324377', '51324378', '51324379', '51324387', '51324388'];
...
// 軌道敷 (JR・新幹線・アストラムの地上部) も建物を通せないので除外対象にする
...
const RAIL_CORRIDORS = [
  { name: 'jr', index: makeIndex(densify(rail.jr.path)), margin: 11 },
  { name: 'shinkansen', index: makeIndex(densify(rail.shinkansen.path)), margin: 10 },
];
// コース路面の半幅 + 余裕
...
function parseAppearance(text) {
  ...
  const app = appEnd < 0 ? text : text.slice(0, appEnd);
  let i = 0;
  while (true) {
...
for (const mesh of MESHES) {
  ...
  const bodyStart = text.indexOf('</app:Appearance>');
  const body = bodyStart < 0 ? text : text.slice(bodyStart);
  let idx = 0, kept = 0;
  while (true) {
